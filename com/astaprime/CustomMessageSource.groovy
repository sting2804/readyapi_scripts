...
CustomMessageSource() {
        ...
        scriptFile = scriptFile.substring(0, scriptFile.indexOf('CustomMessageSource.groovy'))
        //UISupport.showInfoMessage("ScriptPath: $scriptFile")
        properties.load(new FileInputStream(scriptFile + 'messages.properties'))
    }
    ...
    protected String getMessage(String code, Object[] args) {
        ...
        if ((args as List).isEmpty()) {
			...
			if (code.substring(code.indexOf('.')+1) in ['6', '8', '9', '12', '151', '215']) {
               def tmpArgs = args.toList()
               tmpArgs.add(0,'')
               args = tmpArgs as Object[]
           }
