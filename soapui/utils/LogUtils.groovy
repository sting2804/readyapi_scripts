...
class LogUtils
{
   ...
   { 
      def logArea = Logging.logMonitor
      if( logArea != null )
      {
   ...
   def static saveResponseAsFile(WsdlTestCaseRunner testRunner, String testStepName, Logger log)
   {
      ...
      if (testStep == null) {
	     if (log != null)
            log.warn("TestStep '${testStepName}' does not exist. No response will be saved.")
         return
      }
	  if (testStep.testRequest == null) {
	     log.warn("TestStep '${testStepName}' does not have a request. No response will be saved.")
		 return
	  }
